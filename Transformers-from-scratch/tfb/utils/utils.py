...
def sample(logprobs, temperature=1.0):
    ...
    Params:
        lnprobs: Outcome log-probabilities
        temperature: Sampling temperature. 1.0 follows the given distribution, 0.0 returns the maximum probability element
    Returns:
        The index of the sampled element
    """

    if temperature == 0.0:
        return logprobs.argmax()

    p = F.softmax(logprobs / temperature, dim=0)
...
def sample_sequence(model, seed, max_context, length=600, temperature=0.5, verbose=False):
    ...
    Params:
        model
        seed: The sequence to start with
        length: The total number of characters to sample
        temperature: The sampling temperature
        verbose: If true, the sampled sequence is also printed as it is sampled 
    Returns:
        The sampled sequence, including the seed.
    """
...
def sample_batch(data, length, batch_size):
    ...
    Returns:
        A pair (input, target) of integer matrices representing the input and target for the model.
    """
...
def compute_compression(model, data, context, batch_size, verbose=False,
    ...
    Params:
        model: A sequence-to-sequence model that takes as input a (sub) sequence of integers and produces a probability distributuion on the output.
        data: A singe list of integers representing the data
    Returns:
        The result of the computation in "bits per byte". That is, how many bits does the compressed representation spend on each byte (=ASCII character) of the raw data.
    """
    ...
    for current in tqdm.trange(skip, data.size(0)) if verbose else range(skip, data.size(0)):
        ...
        if len(batch) == batch_size or current == data.size(0) - 1:
            ...
            lnprobs = output[torch.arange(b, device=device_()), target_indices, target]
...
def estimate_compression(model, data, nsamples, context, batch_size, verbose=False):
    ...
    Params:
        model: A sequence-to-sequence model that takes as input a (sub) sequence of integers and produces a 
                probability distributuion on the output.
        data: A singe list of integers representing the  data
    Returns:
        The result of the computation in "bits per byte". That is, how many bits does the compressed representation
